...
AI-assisted software delivery. It keeps the original repository theme intact:
...
- [`skills-vnext/`](./skills-vnext/) is the experimental vNext suite
...
Ultra Orchestration does not make an agent "smarter" by only adding more
agents. It makes delivery more reliable by forcing work through a structured
...
The vNext suite strengthens this with:

- design-first hard gates inspired by `obra/superpowers`
- OpenSpec as the durable spec front end and long-term spec back end
...
Use this track when you want the stricter next-generation protocol.
...
### Install Stable Skills

Copy folders from [`skills/`](./skills/) into your agent's global skill
directory.

Windows example for Cursor:

```powershell
Copy-Item -Recurse -Force .\skills\ultra-orchestrator C:\Users\<you>\.cursor\skills\ultra-orchestrator
```

Windows example for Codex project-level skills:

```powershell
Copy-Item -Recurse -Force .\skills\ultra-orchestrator D:\Codex_workspace\.agents\skills\ultra-orchestrator
```

### Install vNext Skills

For a pilot, install the full vNext suite into a project-level skill directory:

```powershell
Copy-Item -Recurse -Force .\skills-vnext\* D:\Codex_workspace\.agents\skills\
```

If you do not want global or workspace installation yet, keep `skills-vnext/`
inside this repository and ask the agent to read the specific `SKILL.md` files
by absolute path.
...
```text
Please read and use:
D:\Codex_workspace\codex_ultra_orchestration\ultra-orchestration\skills-vnext\ultra-vnext-core\SKILL.md
D:\Codex_workspace\codex_ultra_orchestration\ultra-orchestration\skills-vnext\ultra-brainstorming\SKILL.md
D:\Codex_workspace\codex_ultra_orchestration\ultra-orchestration\skills-vnext\ultra-planning\SKILL.md
```
...
- [`skills-vnext/`](./skills-vnext/) 是 vNext 实验技能套件
...
Ultra Orchestration 不是简单增加 agent 数量，而是用工程生命周期提升可靠性：
...
- 吸收 `obra/superpowers` 的 design-first 和 hard gate
- 把 OpenSpec 作为高质量规格前端和长期规格后端
- 用有限状态机闭环替代线性瀑布流
- 用 artifact handoff 替代完整聊天历史继承
- 用 DAG + 写锁定义安全并发
...
| vNext 技能 | 原版本对应技能 | 平替关系 | 核心变化 |
|---|---|---|---|
| `ultra-vnext-core` | `ultra-orchestrator` | 部分平替 + 共享内核 | 把 contracts、状态机闭环、context firewall、安全并发和 host-driven ledger 抽成共享核心。 |
| `ultra-brainstorming` | `clarify-and-intake` + `autoplan` 前半段 | 增强替代 | 增加 design-first、hard gate、逐问澄清、方案对比和规划前批准。 |
| `ultra-planning` | `decision-complete-planner` | 基本平替 | 强化 DAG、owned paths、acceptance checks、retry 假设和串并行边界。 |
| `ultra-execution-control` | `dispatch-and-track` + 部分 `safety-guard` | 增强整合 | 合并 dispatch、写锁、受限重试、blocker、freeze/careful/guard 和 ledger 更新纪律。 |
| `ultra-review` | `spec-review` + `code-review` | 合并平替 | 保留规格一致性和工程质量两层审查，并输出 accept / reject / reroute。 |
| `ultra-qa` | `qa-verify` | 基本平替 | 明确 static QA / dynamic QA 双模式，补充场景覆盖、证据和失败回流。 |
| `ultra-risk-vetting` | `risk-vetter` + `safety-guard` 的审批部分 | 基本平替 | 定义 LOW/MEDIUM/HIGH/EXTREME、审批门槛、护栏映射和 vetter report。 |
| `ultra-delivery` | `deliver-and-retro` | 基本平替 | 统一 final_deliverable、orchestration_log、vetter_report、QA 证据、残余风险和复盘。 |
| `openspec-ultra-bridge-v2` | `openspec-ultra-bridge` | 增强平替 | 保持 OpenSpec 为规格层、Ultra 为控制平面，并用脚本生成 TaskManifest 与 WorkPackage 工件。 |

### vNext 2026-04-23

- 新增 `ultra-risk-vetting`
- 新增 `ultra-delivery`
- README 增加原版到 vNext 平替表
...
把 [`skills/`](./skills/) 里的技能目录复制到目标 agent 的全局或项目技能目录。
...
如果暂时不想安装到全局或工作空间，可以保留在本仓库中，并让 agent 按绝对路径读取对应 `SKILL.md`。
